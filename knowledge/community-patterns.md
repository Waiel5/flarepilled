...
**Takeaway:** R2's zero-egress + S3 API makes it a cheap rclone/restic backup and self-hosted file-sync target where restores are free — add restic/rclone-crypt for versioning and encryption R2 lacks.

In selfhosted circles R2 is a popular cloud backup/sync target precisely because of zero egress: mount a bucket locally with `rclone mount` or push encrypted backups via rclone crypt / restic against R2's S3 API, getting global CDN delivery and free restores (no egress charge to pull your data back). Used as a cheap Dropbox-for-photos / media-library tier and offsite backup. The catch versus B2/S3 is the R2 feature gaps above (no native versioning, placement/IAM limits), so people layer restic's snapshotting or rclone crypt on top for integrity and encryption.
...
**Takeaway:** One DO = one thread (~1k req/s soft cap) and a slow fetch inside it blocks everyone — shard hot keys across many objects instead of one mega-object.

Each Durable Object is single-threaded and cooperatively scheduled (like browser JS), so all requests to one object serialize through one thread. The practical soft ceiling is on the order of hundreds to ~1,000 requests/second for simple ops, and far less if each request does real work. The classic trap is funneling a hot workload (a global counter, a popular chat room, a leaderboard) through ONE object and hitting a wall. Worse: a long-running external fetch() inside a DO blocks every other request to that object for its duration. The fix the community and Cloudflare both endorse is horizontal sharding — spread load across many objects (e.g. Cloudflare's own Queues team sharded DOs for a 10x throughput gain).
...
### ⚠️ avoid · R2's sharp edges vs S3: HTTP Range bug, no region pinning, no IAM
`medium` · R2

**Takeaway:** R2 is not a full S3 replacement: watch for flaky Range requests, no region pinning, no IAM/versioning, and weaker concurrency — validate these before migrating critical paths.

Counterweight to the R2 hype, from devs who hit walls. One reported HTTP Range requests failing ~50% of the time (returning the full object instead of the requested byte range) — CF support called it 'expected behaviour due to a CDN issue' and it was undocumented; this breaks video scrubbing/resumable downloads. Others flag: no object versioning natively (workaround via Workers), no IAM/fine-grained access (no path-scoped access, no SSO/2FA enforcement, no blast-radius limits), unpredictable bucket placement with no region selection ('if you're unlucky it'll be placed in Warsaw'), no cross-region replication, and elevated failure rates on concurrent ListObject. Enterprise/security-sensitive and data-lake-throughput workloads are where people say stay on S3.

Sources: https://news.ycombinator.com/item?id=42256771

### ⚠️ avoid · D1's 10GB-per-database wall forces a sharding mindset
`medium` · D1, Hyperdrive

**Takeaway:** D1 caps each database at 10GB by design — architect for many sharded per-tenant databases up front, or plan an exit to Hyperdrive/Postgres.

Devs evaluating D1 for production repeatedly trip on the 10GB max database size and ask how it's usable at scale. CF's intended answer (and the community's): D1 isn't one big DB — it's meant to be sharded into many small (per-user/per-tenant/per-entity) databases, with thousands of DBs at no extra cost (limit ~50,000 on paid, increases not guaranteed). People who don't design for that from day one face a migration to Hyperdrive+Postgres or external DB once a single tenant's data outgrows 10GB. Same architectural shape as the per-user-DO pattern.
...
### ⚠️ avoid · R2 is missing enterprise S3 features: object lock/versioning, tiering, granular IAM, audit
`medium` · R2

**Takeaway:** Don't treat R2 as a drop-in S3: check for object lock/versioning, IAM granularity, audit, and archival tiering before migrating compliance- or backup-critical buckets.

Same R2 threads carry consistent warnings: R2 lacks (or only recently/partially added) object versioning and object lock — a real risk for accidental-deletion recovery and compliance. No cold storage tier (R2 is ~35% cheaper than S3 Standard but far pricier than Glacier/B2 for archival). Access controls described as 'limited and bolted on' vs S3 IAM granularity + CloudTrail audit. Custom domains require the domain to be on Cloudflare. Some reported write latency near ~1s vs S3's <100ms, and R2 scoring worst in S3-compat benchmarks for some ops. Verdict: great for hot data needing egress; for archival use B2 (cited ~4x cheaper) or Glacier; for same-region AWS compute, S3 egress is already free.

Sources: https://news.ycombinator.com/item?id=38118577 · https://news.ycombinator.com/item?id=33453104

### ⚠️ avoid · D1 single-primary-region latency bites: 200–500ms+ reads from far edges, no edge result cache
`medium` · D1

**Takeaway:** D1 is single-region under the hood — expect WAN latency on cache-miss reads from distant edges; don't assume 'edge DB' means low global latency.

The 'Optimize D1 Queries' thread surfaced multiple year-plus production reports of high latency: 400ms+ (spiking >500ms) for simple queries, 200ms+ from North America, and 300–3000ms vs 50–100ms on DigitalOcean Postgres. Root cause is architectural: a D1 DB lives in ONE primary region, so every cache-miss from Cloudflare's 300+ edges round-trips to that region, and there's no active edge result caching. Despite running 'at the edge,' compute and DB can be far apart. Community consensus: D1 currently fits small, low-complexity, read-light projects (visitor tracking, mailing lists) rather than latency-sensitive global apps.
...
### ⚠️ avoid · Dzero: 'Supabase for SQLite' on D1 — but global replication is just storage duplication, not read replicas
`community` · D1, R2

**Takeaway:** Scrutinize 'globally distributed SQLite' claims — without true read replicas it's file duplication with hand-managed consistency, and D1's 10GB/download limits still apply.

Show HN Dzero layers a dashboard + API over Cloudflare D1, pitching globally distributed SQLite by duplicating the SQLite file across R2/S3 instances. Under scrutiny the author conceded D1 has no read replicas, so '320 edge locations' is achieved by duplicating files and 'managing consistency' manually — raising unanswered consistency-guarantee questions. The 10GB/DB cap is unsolved (sharding only planned), and full-DB download is limited for large DBs. Commenters likened it to a thin layer over an existing service (the Vercel critique) and questioned defensibility if Cloudflare ships similar tooling. Useful as a cautionary tale on 'distributed SQLite' claims.
